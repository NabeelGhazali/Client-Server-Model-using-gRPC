...
server.addService(proto.BatchTransferService.service, {
    Batches: (RFW, callback) => {
        ...
        var lengthOfBatch = RFW.request.BatchUnit;

        var batch = [];

        for(j=0; j<mydata.length; j++){
            for(i=0; i<lengthOfBatch; i++){
                var datapoint = {
        ...
        callback(null, {message: "Batch created Succesfully"})
    },
    MyBatches: (RFW, callback) => {
        ...
        var analyzeValue = anaylze(workloadlist, RFW.request.DataAnalytics)

        RFD = {"RFW_ID": RFW.request.RFW_ID,
               "BatchID": batchID+numberOfBatches,
               "batches": multiplebatches,
               "analytics": analyzeValue}
        try{
            callback(null, RFD)
        } catch(error){
            callback(error, null)
...
function anaylze(list, analysis) {
    if (analysis == "avg")
        return average(list)
...
server.bindAsync("127.0.0.1:5000", 
